        ...
        {messages.map((msg) => (
          ...
          >
            ...
            >
                  ...
                  <div className="space-y-3">
                    {msg.data.recommendations?.map((job: any) => (
                      <div key={job.id} className="p-4 rounded-xl bg-card/60 border border-border/80 space-y-3 hover:border-primary/50 transition-colors">
                        ...
                        {/* Direct Action Chips per Job */}
                        <div className="flex items-center gap-2 flex-wrap pt-2 border-t border-border/60 font-sans">
                          <button
                            onClick={() => handleSendPrompt(`Tailor CV and resume for ${job.company}`)}
                            className="px-3 py-1.5 rounded-lg bg-primary/10 hover:bg-primary/20 text-primary border border-primary/30 text-[11px] font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
                          >
                            <FileText className="w-3.5 h-3.5" /> Tailor CV & Resume
                          </button>
                          <button
                            onClick={() => handleSendPrompt(`Send email to HR for ${job.company}`)}
                            className="px-3 py-1.5 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-500 border border-emerald-500/30 text-[11px] font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
                          >
                            <Mail className="w-3.5 h-3.5" /> Send Mail to HR
                          </button>
                          <button
                            onClick={() => handleSendPrompt(`Apply job for ${job.company}`)}
                            className="px-3 py-1.5 rounded-lg bg-amber-500/10 hover:bg-amber-500/20 text-amber-500 border border-amber-500/30 text-[11px] font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
                          >
                            <Briefcase className="w-3.5 h-3.5" /> Apply Job
                          </button>
                          <button
                            onClick={() => handleSendPrompt(`Find referrals for ${job.company}`)}
                            className="px-3 py-1.5 rounded-lg bg-card border border-border/80 hover:bg-muted text-foreground text-[11px] font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
                          >
                            <Users className="w-3.5 h-3.5 text-primary" /> Get Referral Details
                          </button>
                        </div>
                      </div>
                    ))}
              ...
              {/* DYNAMIC RESULT: TAILORED CV RESULT */}
              {msg.data?.type === 'TAILOR_RESUME_RESULT' && (
                <div className="space-y-3 pt-2">
                  <div className="p-4 rounded-xl bg-primary/10 border border-primary/30 space-y-3 font-sans">
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="font-bold text-foreground text-sm block">{msg.data.resume.company}</span>
                        <span className="text-xs text-muted-foreground">{msg.data.resume.title}</span>
                      </div>
                      <span className="px-2.5 py-1 rounded-lg text-xs font-bold font-mono bg-emerald-500/10 text-emerald-500 border border-emerald-500/30">
                        ATS: {msg.data.resume.ats_score}%
                      </span>
                    </div>

                    <div className="p-3 rounded-xl bg-card/80 border border-border/80 space-y-1.5 text-xs">
                      <span className="text-primary font-bold text-[10px] uppercase font-mono block">// TAILORING HIGHLIGHTS</span>
                      {msg.data.resume.highlights?.map((h: string, i: number) => (
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      <a
                        href={msg.data.resume.download_url}
                        download
                        className="px-4 py-2 bg-primary text-primary-foreground font-bold text-xs rounded-xl shadow-xs flex items-center gap-1.5 transition-colors cursor-pointer hover:bg-primary/90"
                      >
                        <Download className="w-4 h-4" /> Download Tailored CV (PDF)
                      </a>
                    </div>
                  </div>
                </div>
              ...
              {/* DYNAMIC RESULT: HR EMAIL DISPATCHED */}
              {msg.data?.type === 'SEND_HR_EMAIL_RESULT' && (
                <div className="space-y-3 pt-2">
                  <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 space-y-2.5 font-sans">
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="font-bold text-foreground text-sm block">{msg.data.details.company}</span>
                        <span className="text-xs text-muted-foreground">Recipient HR: {msg.data.details.hr_email}</span>
                      </div>
                      <span className="px-2.5 py-1 rounded-lg text-[10px] font-bold font-mono bg-emerald-500/20 text-emerald-400 border border-emerald-500/40">
                        EMAIL_DISPATCHED
                      </span>
                    </div>
                    <div className="p-3 rounded-xl bg-card/80 border border-border/80 space-y-1 text-xs font-mono">
                      <div><strong>Subject:</strong> {msg.data.details.subject}</div>
                      <div><strong>Attachment:</strong> {msg.data.details.attached_file}</div>
                    </div>
                  </div>
                </div>
              )}

              {/* DYNAMIC RESULT: JOB APPLIED */}
              {msg.data?.type === 'APPLY_JOB_RESULT' && (
                <div className="space-y-3 pt-2">
                  <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 space-y-2.5 font-sans">
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="font-bold text-foreground text-sm block">{msg.data.application.company}</span>
                        <span className="text-xs text-muted-foreground">{msg.data.application.job_title}</span>
                      </div>
                      <span className="px-2.5 py-1 rounded-lg text-[10px] font-bold font-mono bg-emerald-500/10 text-emerald-500 border border-emerald-500/30">
                        STATUS: SUBMITTED
                      </span>
                    </div>
                    {msg.data.application.apply_url && (
                      <a
                        href={msg.data.application.apply_url}
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-1.5 text-xs text-primary font-bold hover:underline font-mono"
                      >
                        Open Direct Application Portal <ExternalLink className="w-3.5 h-3.5" />
                      </a>
                    )}
                  </div>
                </div>
