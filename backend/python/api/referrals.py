...
def list_referral_opportunities(
    ...
    status: Optional[str] = Query(None, description="Filter by status e.g. READY_FOR_REVIEW, SENT, QUALIFIED"),
...
async def trigger_referral_discovery(threshold: Optional[int] = Query(90, ge=70, le=100)):
    ...
    Prioritizes 1st-degree LinkedIn connections.
    """
...
async def generate_message_for_referral(referral_id: str, req: GenerateMessageRequest = Body(default=GenerateMessageRequest())):
    ...
    referral_repository.log_audit(
        ...
        details=f"Personalized message generated ({msg_res.get('generated_by', 'AI')})."
    )
...
def edit_referral_message(referral_id: str, req: EditMessageRequest):
    ...
    updated = referral_repository.update_referral_status(
        referral_id=referral_id,
        status=ref.get("status", "READY_FOR_REVIEW"),
        message=req.message
    )
    referral_repository.log_audit(
        referral_id=referral_id,
...
async def send_referral_outreach(referral_id: str, req: SendOutreachRequest = Body(default=SendOutreachRequest())):
    """Dispatches referral message to verified contact."""
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to dispatch referral: {e}")
